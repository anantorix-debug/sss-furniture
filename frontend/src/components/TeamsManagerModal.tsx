...
const emptyForm = { name: '', workerType: 'CARPENTER' as WorkerType, groupId: '', groupName: '' };
...
export function TeamsManagerModal({ onClose, onChange }: { onClose: () => void; onChange: () => void }) {
  ...
  function startEdit(team: ProductionTeam) {
    setEditing(team);
    setForm({ name: team.name, workerType: team.workerType, groupId: team.groupId ?? '', groupName: team.groupName ?? '' });
    setError(null);
  }
  ...
  return (
    <Modal title="Manage Production Teams" onClose={onClose} wide>
